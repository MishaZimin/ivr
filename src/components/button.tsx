...
function RandomImage({ position }: { position: Position }) {
    return (
        <img
            ...
            className="absolute w-20 rounded-xl shadow-md"
        />
    );
}
    ...
    return (
        <>
            <div className="h-screen flex items-center justify-center">
                <button
                    className="bg-white text-black px-5 py-3 rounded-xl min-w-28 shadow-md"
                    onClick={handleClick}
                >
                    click {count}
                </button>

                {count > 1 && <RandomImage position={imagePosition} />}
            </div>
        </>
    );
}
